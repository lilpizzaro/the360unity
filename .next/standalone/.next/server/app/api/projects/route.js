...
async function GET(request) {
    ...
    try {
        ...
        // Get all projects
        let query = supabase/* supabase */.ND.from('projects').select('*');
        if (category && category !== "all") {
            query = query.eq('category', category);
        }
        if (technology && technology !== "All") {
            // This assumes technologies is stored as an array in Supabase
            query = query.contains('technologies', [
                technology
            ]);
        }
        if (search) {
        ...
        let userStars = {};
        if (userId) {
            const { data: starredData, error: starredError } = await supabase/* supabase */.ND.from('project_likes').select('project_id').eq('user_id', userId);
            if (!starredError && starredData) {
                userStars = starredData.reduce((acc, item)=>{
...
async function POST(request) {
    ...
    try {
        ...
        const { data: newProject, error } = await supabase/* supabase */.ND.from('projects').insert(projectData).select().single();
        if (error) {
        ...
        return server.NextResponse.json(newProject);
    } catch (error) {
...
;// ./node_modules/next/dist/build/webpack/loaders/next-app-loader/index.js?page=%2Fapi%2Fprojects%2Froute&name=app%2Fapi%2Fprojects%2Froute&pagePath=private-next-app-dir%2Fapi%2Fprojects%2Froute.ts&appDir=D%3A%5Cthe360unity-master%5Cthe360unity-master%5Csrc%5Capp&appPaths=%2Fapi%2Fprojects%2Froute&pageExtensions=tsx&pageExtensions=ts&pageExtensions=jsx&pageExtensions=js&basePath=&assetPrefix=&nextConfigOutput=standalone&nextConfigExperimentalUseEarlyImport=&preferredRegion=&middlewareConfig=e30%3D!
...
const routeModule = new module_compiled.AppRouteRouteModule({
    ...
    resolvedPagePath: "D:\\the360unity-master\\the360unity-master\\src\\app\\api\\projects\\route.ts",
    nextConfigOutput,
...
var __webpack_exports__ = __webpack_require__.X(0, [4447,2190,7296,8443,9398], () => (__webpack_exec__(89674)));
